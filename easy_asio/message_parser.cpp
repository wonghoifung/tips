...
public:
	concrete_message_parser(message_parser_handler * h):message_parser(h)
	{
		buf_ = inmsg_.buffer();
		reset();
	}
	...
	int parse(const char* data, const size_t length)
	{
		...
		while(ndx < length && state_ != state_error)
		{
			...
			{
				handler_->on_parse_complete(&inmsg_);
				reset();
			}
		}
...
message_parser* message_parser::create(message_parser_handler* h)
{
