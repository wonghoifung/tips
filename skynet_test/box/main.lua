...
if mode == "agent" then
	...
	skynet.fork(function()
		...
		local greeting = socket.read(cid)
		if greeting then
		...
		local start = socket.read(cid)
		if start then
			print("----> server agent start")
		else
			print("----> error signal: "..start)
			return
		end
		local counter = 0
		while true do
			...
			socket.write(cid, "hello")
			local res = socket.read(cid)
			if res then
				print("----> recv: "..res..", current: "..skynet.time())
				skynet.sleep(100)
			else
				socket.close(cid)
				break
