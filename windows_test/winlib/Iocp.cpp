...
CIocp::CIocp():mIocpHandle(NULL),mWorkerThreadCount(0),mStartupEventHandle(NULL)
{
}
...
{
	if (!socket || !completionKey) return FALSE;
...
VOID CIocp::WorkerThreadCallback()
{
	...
	while (TRUE)
	{
		SetEvent(mStartupEventHandle); // TODO move outside the while loop ?
